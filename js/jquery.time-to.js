 ...
 * @date 2018-12-15
 *
 * Pascal : added Pause/restart, up/down timer/abbr_day
 */
...
(function (factory) {
    ...
    var methods = {
        start: function (sec,resume) {
            var intervalId;
            if (sec || ((sec == 0) && !this.data('countdown')) ) {
				if (!this.data('countdown')  && !resume) {
					init.call(this, 0);
				} else {
					init.call(this, sec);
				}
        ...
        resume: function () { // Pascal: resume added
            var data = this.data();
			t = 0; // compute current timer value
			for (i = 0; i < data.vals.length-1; i += 2) { 
				t = (t*60) + (data.vals[i]*10) + (data.vals[i+1]*1);
	        } 
			data.seconds = t;
			data.sec = t;
			methods.start.call(this,t,true);
		}
    ...
    /* pascal : add abbr_day */
    var dictionary = {
        en: { days: 'days',   hours: 'hours',  min: 'minutes',  sec: 'seconds', abbr_day: "d" },
        ru: { days: 'дней',   hours: 'часов',  min: 'минут',    sec: 'секунд' , abbr_day: "д"},
        ua: { days: 'днiв',   hours: 'годин',  min: 'хвилин',   sec: 'секунд' , abbr_day: "д"},
        de: { days: 'Tag',    hours: 'Uhr',    min: 'Minuten',  sec: 'Secunden' , abbr_day: "T"},
        fr: { days: 'jours',  hours: 'heures', min: 'minutes',  sec: 'secondes', abbr_day: "j" },
        sp: { days: 'días',   hours: 'horas',  min: 'minutos',  sec: 'segundos' , abbr_day: "d"},
        it: { days: 'giorni', hours: 'ore',    min: 'minuti',   sec: 'secondi' , abbr_day: "g" },
        nl: { days: 'dagen',  hours: 'uren',   min: 'minuten',  sec: 'seconden' , abbr_day: "d"},
        no: { days: 'dager',  hours: 'timer',  min: 'minutter', sec: 'sekunder' , abbr_day: "d"},
        pt: { days: 'dias',   hours: 'horas',  min: 'minutos',  sec: 'segundos', abbr_day: "d" },
        tr: { days: 'gün',    hours: 'saat',   min: 'dakika',   sec: 'saniye' , abbr_day: "g"},
        pl: { days: 'dni', hours: 'godziny', min: 'minuty', sec: 'secundy' , abbr_day: "d"}
    };
    ...
    $.fn.timeTo = function () {
        ...
        if (options.countdown !== false
            ...
            options.displayDays = (days < 10 && 2) || (days < 100 && 2) || 3; // pascal: day lgth = 1 => force 2
        ...
        return this.each(function () {
            ...
            if (!data.vals) {
                ...
                if (data.displayDays > 0) {
                    ...
                    dhtml = dhtml1;
					// pascal : add abbr_day if !displaycaptions
					data.displayCaptions ? dot2 = "" 
						: dot2 = '<span style="margin-right:0' + Math.round(marginRight) + 'px">'+language.abbr_day+'</span>';
    ...
    function tick(digit) {
        ...
        if ($.support.transition) {
            ...
            setTimeout(function () {
                ...
                if (((step < 0) && (data.sec === data.countdownAlertLimit)) || 
					((step > 0) && (t == (data.options.seconds - data.countdownAlertLimit))))	{
                    $digits.parent().addClass('timeTo-alert');
                }

				if ( ((step < 0) &&(data.sec === 0)) || ((step > 0) && (t === data.options.seconds)) ) {
